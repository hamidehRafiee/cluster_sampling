...
def eval_metrics_one_dataset(model, test_dataloader, device, k_vals, opt):
    ...
    with torch.no_grad():
        ...
        # print(model_generated_cluster_labels.reshape(-1))
        # ## Compute NMI
        NMI = metrics.cluster.normalized_mutual_info_score(
            model_generated_cluster_labels.reshape(-1), target_labels.reshape(-1))
        ...
        # print(k_closest_points.shape)
        # print(k_closest_points)
        # print(k_closest_points[0].shape)
        # print(k_closest_points[0])
        # print(target_labels)
        # print(target_labels.reshape(-1))
        # print(k_closest_points[0,1:])
        k_closest_classes = target_labels.reshape(-1)[k_closest_points[:, 1:]]
        # print(k_closest_classes.shape)
        # print(k_closest_classes)
        # print(k_closest_classes[0].shape)
        # print(k_closest_classes[0])
        # ## Compute Recall
        recall_all_k = []
        for k in k_vals:
            recall_at_k = np.sum(
                [1 for target, recalled_predictions in zip(target_labels, k_closest_classes) if
                 target in recalled_predictions[:k]]) / len(target_labels)
            recall_all_k.append(recall_at_k)

        # ## Compute F1 Score
        F1 = f1_score(model_generated_cluster_labels, target_labels, feature_coll,
                      computed_centroids)

    return F1, NMI, recall_all_k, feature_coll


def eval_metrics_query_and_gallery_dataset(model, query_dataloader, gallery_dataloader, device,
                                           k_vals, opt):
    """
    Compute evaluation metrics on test-dataset, e.g. NMI, F1 and Recall @ k.
    ...
    with torch.no_grad():
        ...
        query_target_labels, query_feature_coll = np.hstack(query_target_labels).reshape(-1,
                                                                                         1), np.vstack(
            query_feature_coll).astype('float32')
        gallery_target_labels, gallery_feature_coll = np.hstack(gallery_target_labels).reshape(-1,
                                                                                               1), np.vstack(
        ...
        for k in k_vals:
            recall_at_k = np.sum(
                [1 for target, recalled_predictions in zip(query_target_labels, k_closest_classes)
                 if target in recalled_predictions[:k]]) / len(query_target_labels)
            recall_all_k.append(recall_at_k)
        recall_str = ', '.join(
...
def recover_closest_one_dataset(feature_matrix_all, image_paths, save_path, n_image_samples=10,
                                n_closest=3):
    """
    ...
    # print(closest_feature_idxs)
    # print(closest_feature_idxs.shape)
    # print(image_paths) # list of image's paths
    # print(image_paths.shape) # (5924,)
    # print(image_paths[closest_feature_idxs])
    # print(image_paths[closest_feature_idxs].shape)
    # print(image_paths[closest_feature_idxs][sample_idxs])
...
def recover_closest_one_dataset_myself(feature_matrix_all, image_paths, save_path, n_image_samples=15,
                                n_closest=5):

    image_paths = np.array([x[0] for x in image_paths])
    #8131
    sample_idxs = np.array([4131, 4533, 1953, 2448, 2633, 2958, 3273, 3495, 8027, 8107, 7808, 7441,
                            100, 1000, 4300])
...
####### RECOVER CLOSEST EXAMPLE IMAGES #######
def recover_closest_inshop(query_feature_matrix_all, gallery_feature_matrix_all, query_image_paths,
                           gallery_image_paths, save_path, n_image_samples=10, n_closest=3):
    """
    Provide sample recoveries.
    ...
    Returns:
        Nothing!
    """
    query_image_paths, gallery_image_paths = np.array(query_image_paths), np.array(
        gallery_image_paths)
...
class InfoPlotter():
    ...
    def make_plot(self, t_epochs, v_epochs, t_metrics, v_metrics, t_labels, v_labels,
                  appendix=None):
        """
        ...
        for i in range(len(t_metrics)):
            axes[0].plot(t_epochs, t_metrics[i], '-{}'.format(self.t_colors[i]), linewidth=1,
                         label=t_labels[i])
        ...
        for i in range(len(v_metrics)):
            axes[1].plot(v_epochs, v_metrics[i], '-{}'.format(self.v_colors[i]), linewidth=1,
                         label=v_labels[i])
...
def set_logging(opt):
    ...
    if opt.savename == '':
        ...
        time_string = '{}-{}-{}-{}-{}-{}'.format(date.year, date.month, date.day, date.hour,
                                                 date.minute, date.second)
        checkfolder = opt.save_path + '/{}_{}_'.format(opt.dataset.upper(),
                                                       opt.arch.upper()) + time_string
...
class LOGGER():
    ...
    def __init__(self, opt, metrics_to_log, name='Basic', start_new=True):
        ...
        if self.prop.dataset != 'vehicle_id':
            ...
            self.info_plot2 = InfoPlotter(opt.save_path + '/InfoPlot_2_{}.svg'.format(name))
        else:
            self.info_plot = {'Set {}'.format(i): InfoPlotter(
                opt.save_path + '/InfoPlot_{}_Set{}.svg'.format(name, i + 1)) for i in range(3)}
        ...
        # ## Set CSV Writters
        self.csv_loggers = {
            mode: CSV_Writer(opt.save_path + '/log_' + mode + '_' + name + '.csv', lognames) for
    ...
    def provide_progress_saver(self, metrics_to_log):
        ...
        """
        Progress_Saver = {key: {sub_key: [] for sub_key in metrics_to_log[key]} for key in
                          metrics_to_log.keys()}
    ...
    def update_info_plot(self):
        ...
        t_loss_list = [self.progress_saver['train']['Train Loss1'],self.progress_saver['train']['Train Loss2']]
        t_legend_handles = ['Train Loss1','Train Loss2']
        ...
        if self.prop.dataset != 'vehicle_id':
            title = ' | '.join(key + ': {0:3.3f}'.format(np.max(item)) for key, item in
                               self.progress_saver['val'].items() if key not in ['Time', 'Epochs'])
            self.info_plot.title = title
            self.info_plot2.title = title
            v_metric_list = [self.progress_saver['val'][key] for key in
                             self.progress_saver['val'].keys() if key not in ['Time', 'Epochs']]
            v_legend_handles = [key for key in self.progress_saver['val'].keys() if
                                key not in ['Time', 'Epochs']]

            v_metric_list2 = [self.progress_saver['val2'][key] for key in
                             self.progress_saver['val2'].keys() if key not in ['Time', 'Epochs']]
            v_legend_handles2 = [key for key in self.progress_saver['val2'].keys() if
                                key not in ['Time', 'Epochs']]

            self.info_plot.make_plot(t_epochs, v_epochs, t_loss_list, v_metric_list,
                                     t_legend_handles, v_legend_handles)

            self.info_plot2.make_plot(t_epochs, v_epochs, t_loss_list, v_metric_list2,
                                     t_legend_handles, v_legend_handles2)
        else:
            # Iterate over all test sets.
            for i in range(3):
                title = ' | '.join(key + ': {0:3.3f}'.format(np.max(item)) for key, item in
                                   self.progress_saver['val'].items() if
                                   key not in ['Time', 'Epochs'] and 'Set {}'.format(i) in key)
                self.info_plot['Set {}'.format(i)].title = title
                v_metric_list = [self.progress_saver['val'][key] for key in
                                 self.progress_saver['val'].keys() if
                                 key not in ['Time', 'Epochs'] and 'Set {}'.format(i) in key]
                v_legend_handles = [key for key in self.progress_saver['val'].keys() if
                                    key not in ['Time', 'Epochs'] and 'Set {}'.format(i) in key]
                self.info_plot['Set {}'.format(i)].make_plot(t_epochs, v_epochs, t_loss_list,
                                                             v_metric_list, t_legend_handles,
                                                             v_legend_handles,
...
def metrics_to_examine(dataset, k_vals):
    ...
    """
    metric_dict = {'train': ['Epochs', 'Time', 'Train Loss1', "train Loss2"]}
...
def save_graph(opt, model):
    ...
    def make_dot(var, savename, params=None):
        ...
        node_attr = dict(style='filled',
                         ...
                         height='0.6',
                         width='1')
        dot = Digraph(node_attr=node_attr, format='svg',
                      graph_attr=dict(size="40,10", rankdir='LR', rank='same'))
        seen = set()
    ...
    viz_graph = make_dot(network_output,
                         opt.save_path + "/Network_Graphs" + "/{}_network_graph".format(opt.arch))
...
def eval_metrics_one_dataset(model, test_dataloader, device, k_vals, opt, dim, LOG):
    """
    Compute evaluation metrics on test-dataset, e.g. NMI, F1 and Recall @ k.
    ...
    with torch.no_grad():
        ...
        for idx, inp in enumerate(final_iter):
            ...
            target_labels.extend(target.numpy().tolist())
            if dim==0:
                out, _, _ = model(input_img.to(device))
            elif dim == 1:
                _, out1, out2 = model(input_img.to(device))
        ...
        feature_coll = np.vstack(feature_coll).astype('float32')
        if dim == 0:
            np.save(LOG.prop.save_path + "/test_embedding.npy", feature_coll)
            np.save(LOG.prop.save_path + "/test_label.npy", target_labels)
...
class LOGGER():
    ...
    def __init__(self, opt, metrics_to_log, name='Basic', start_new=True):
        ...
        if self.prop.dataset != 'vehicle_id':
            self.info_plot = InfoPlotter(opt.save_path + '/InfoPlot_{}.svg'.format(name))
            self.info_plot2 = InfoPlotter(opt.save_path + '/InfoPlot2_{}.svg'.format(name))
        else:
            self.info_plot = {'Set {}'.format(i): InfoPlotter(
                opt.save_path + '/InfoPlot_{}_Set{}.svg'.format(name, i + 1)) for i in range(3)}
    ...
    def update_info_plot(self):
        ...
        """
        t_epochs = self.progress_saver['val']['Epochs']
        t_loss_list = [self.progress_saver['train']['Train Loss']]#, self.progress_saver['train']['Train Loss2']
        t_loss_list2 = [self.progress_saver['train']['Train Loss']]
        t_legend_handles = ['Train Loss']#, 'Train Loss2'
        t_legend_handles2 = ['Train Loss']
        ...
        if self.prop.dataset != 'vehicle_id':
            ...
            self.info_plot.make_plot(t_epochs, v_epochs, t_loss_list, v_metric_list,
                                     t_legend_handles, v_legend_handles)
            # self.info_plot2.make_plot(t_epochs, v_epochs, t_loss_list, v_metric_list,
            #                          t_legend_handles, v_legend_handles)
        else:
            # Iterate over all test sets.
            for i in range(3):
...
def metrics_to_examine(dataset, k_vals):
    ...
    Returns:
        metric_dict: Dictionary representing the storing structure for LOGGER.progress_saver. See LOGGER.__init__() for an example.
    """
    metric_dict = {'train': ['Epochs', 'Time', 'Train Loss']}#, "Train Loss2"
    ...
    if dataset == 'vehicle_id':
        ...
        metric_dict['val2'] = ['Epochs', 'Time', 'NMI', 'F1']
        metric_dict['val2'] += ['Recall @ {}'.format(k) for k in k_vals]
